...
distrib(kappa,beta,typetag,generator,level,nreb)=yodaget(sprintf("mMDT_GA_%02d_%02d_Q%d_R%d",10*kappa,10*beta,Q,10*R), '../'.generator.'/lhc/'.level.'/'.typetag.'.yoda').' | ./rebin.pl -2 -rc '.nreb
...
do for [itype=1:words(types)]{
    ...
    do for [jtype=1:words(levels)]{
        ...
        # same file
        set out 'distributions-'.typetag.'-mmdt-pp.pdf'
        set title '{/:Bold '.type.', '.level.', with mMDT} {/: }'
