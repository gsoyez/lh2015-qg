...
for measure in measures:
    for observable in observables:
        ...
        for Rval in Rvalues:
            command="./get-separation.sh "+table+" "+observable+"_Q"+str(Qvalue)+"_R"+str(Rval)+" "+measure
            separation_scatter.addPoint(0.1*Rval, float(os.popen(command).read().rstrip()), xerrs=0.1)
        scatters.append(separation_scatter)

for measure in measures:
    for observable in observables:
        separation_scatter = yoda.Scatter2D(title=measure, path="/Rdependence/mMDT_"+measure+"_"+observable)
        for Rval in Rvalues:
            command="./get-separation.sh "+table+" mMDT_"+observable+"_Q"+str(Qvalue)+"_R"+str(Rval)+" "+measure
            separation_scatter.addPoint(0.1*Rval, float(os.popen(command).read().rstrip()), xerrs=0.1)
        scatters.append(separation_scatter)
