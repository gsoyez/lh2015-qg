...
\ee
where $C_i$ is the color factor, exactly as needed for
\Eq{eq:casimirform}.  This LL result can be understood from the fact
that quarks and gluons have identical leading splitting functions
($P(z) \simeq 2/z$ \jdt{factor of two correct?} \gs{I think that
  \eqref{eq:LLform} is correct but I'd define $P=2C_i/z$, implynig a
  sentence like ``the splitting functions are te same up to the
  overall multiplicative colour factor''}
...
\ee
This function goes linearly to zero at $\epsilon \to 0$, falls
exponentially as $\delta \to \infty$ \gs{you mean $\epsilon\to\infty$,
  right?}, is normalized by $\int \df \epsilon \,  F(\epsilon) = 1$, and has an expectation value $\vev{\epsilon} = \int \df \epsilon \, \epsilon \, F(\epsilon) = \epsilon_0$.  The parameter $\epsilon_0$ is the average shift of the perturbative distribution.\footnote{In \Ref{}, an alternative form of $F(\epsilon) = (4\epsilon/\epsilon_0^2) e^{-2 \epsilon/\epsilon_0}$ was proposed which has the same rough features.  We find that this functional form has a large of a high-side tail and gives rather unrealistic angularity distributions for gluons.}
