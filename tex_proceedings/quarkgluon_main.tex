...
\caption{Hadron-level distributions of the LHA for (a) the $e^+ e^- \to u \bar{u}$ quark-tagged sample, (b) the $e^+ e^- \to gg$ gluon-tagged sample, and (c) the classifier separation integrand in Eq.~\eqref{quarkgluon_eq:deltaintegrand}.  Six parton shower generators---\textsc{Pythia 8.205}, \textsc{Herwig++ 2.7.1}, \textsc{Sherpa 2.1.1}, \textsc{Vincia 1.201}, \textsc{Deductor 1.0.2}, and \textsc{Ariadne} \jdt{what version?}---are run at their baseline settings with center-of-mass energy $Q = 200~\GeV$ and jet radius $R= 0.6$.  \jdt{Need to figure out color scheme, labeling, and what happened to Deductor curves.}}
...
Our \textsc{Deductor} baseline uses leading color plus (LC+) matrix elements, which includes some subleading color structures.   When Deductor interfaces with \textsc{Pythia} for hadronization, LC+ matrix elements are used but only leading color (LC) information is passed to the Lund string model.  Switching from LC+ to LC matrix elements at parton level has a negligible impact on quark/gluon discrimination power, so no  \textsc{Deductor} variations are shown in this study.  \jdt{If we could have $g \to q \bar{q}$ turned off for Deductor, that would be very helpful.}
