...
, \quad \text{mass} .
\end{equation}
\gs{Do we want to qupte the values of $(\kappa,\beta)$ for each
  variabke?} Here, multiplicity is the hadron multiplicity within the
...
(gluon-tagged) sample.  \gs{This and other potential...} Other
potential performance metrics are discussed in
...
gluons labels without reference to the final state. \gs{I'd clarify
  this a bit. In principle, the quantum numbers of the associated
  colour singlet are not measurable event by event. I guess that the
  idea here is that with ultimate knowledge you could do it (without
  referring to QCD) so it is at least defined in a fundamental way.}  We
compare four different parton shower generators both before
...
non-perturbative hadronization plays an important role. \gs{the
  variables are not precisely defined yet, so this might not be clear
  for everyone.} Surprisingly, comparably-sized differences are also
...
this study is that, contrary to the standard lore, \gs{existing}
...
\gs{(approximate)} order from most ill-defined to most well-defined.
...
\item \textbf{An initiating quark parton in a final state parton
  ...
  experimentalists have in mind.  This is sometimes referred to as the
  ``max-$p_T$'' prescription \gs{this can be a bit misleading since
    the max-$p_t$ parton can be different from the ``initiating q/g''
    (although the difference is calculable in a LL shower, see
    e.g. arXiv:1411.5182). Why not move the max-$p_t$ as a separate
    bullewt?} (see further discussion in \cite{Buckley:2015gua}),
  though it assumes that the parton shower history is meaningful,
...
\item \textbf{A phase space region (as defined by an unambiguous
  ...
  the discriminant variables of interest. \gs{The next two sentences
    are important. I think they are related to Dave's comments (and
    the text below clarifies things). Is there a way to give them more
    prominence (e.g. put them in italic)?}  Of course, the main
...
For example, the user could choose that small $\lambda$ jets should be tagged as ``quark-like'' while large $\lambda$ jets should be tagged as ``gluon-like''. Alternatively, the user might combine $\lambda$ with other discriminant variables as part of a more sophisticated classification scheme.  The key point is that one first measures hadron-level discriminant variables on a final state of interest, and only later does one interpret exactly what those discriminants accomplish (which could be different depending on this physics goals of a specific analysis).
...
parton shower tuning. \gs{and can act as a powerful discriminant in
  BSM searches.}
...
\gs{3-}momentum $\vec{p}$; this turns out to be a desirable feature
...
\end{equation}
\gs{Do you want to move parts of this to the previous Section
  (e.g. saying that (\ref{quarkgluon_eq:deltadef}) has the advantage
  over other classifiers that we can directly look at the integrand to
  see which region contributes the most.} \gs{If we know the region
  that contributes, we can see if it is under control in the Monte
  Carlos and how big are the NP corrections [Note: I have not yet read
  the draft until the end, so discard this if the answer is provided
  later, or maybe just mention that
  (\ref{quarkgluon_eq:deltaintegrand}) can also help in that
  respect.]}  This quantity shows where in the LHA phase space the
actual discrimination power lies, with large values of the integrand
...
extremes and similar to each other.
...
LHA from the perturbative shower alone. \gs{Could that be related to
  differences in the shower cut-off?} Since
