...
\caption{Average nonperturbative shifts to the IRC safe angularities as a
  ...
  trend agrees, the hadronization corrections (as implemented in the parton showers) do not appear to exhibit Casimir scaling.  \jdt{Gregory recommends showing ratio as well, but I think I disagree.  To be discussed.}}
\label{fig:NPshift}
\end{figure}
...
2.001~\cite{Fischer:2016vfv}, which includes LO matrix-element corrections  (see \cite{Giele:2011cb}) up
to ${\cal O}(\alpha_s^3)$ for both $e^+ e^- \to q \bar{q}$ and $e^+ e^- \to gg$.  In \Fig{fig:settings_variation_vincia}, we consider the following \textsc{Vincia} variations: 
\begin{itemize}
