...
\end{itemize}
To test other generators, the analysis code used for this study is available as a \textsc{Rivet} routine \cite{Buckley:2010ar}, which can be downloaded from \verb|https://github.com/gsoyez/lh2015-qg|.
...
\caption{Hadron-level distributions of the LHA for (a) the $e^+ e^- \to u \bar{u}$ (``quark jet'') sample, (b) the $e^+ e^- \to gg$ (``gluon jet'') sample, and (c) the classifier separation integrand in \Eq{eq:deltaintegrand}.  Seven parton shower generators---\textsc{Pythia 8.205}, \textsc{Herwig++ 2.7.1}, \textsc{Sherpa 2.1.1}, \textsc{Vincia 1.201}, \textsc{Deductor 1.0.2}, \textsc{Ariadne 5.0.$\beta$}, and \textsc{Dire 1.0.0} \jdt{Remember to update versions}---are run at their baseline settings with center-of-mass energy $Q = 200~\GeV$ and jet radius $R= 0.6$.  We also show the analytic NLL results from \Sec{sec:analytic}.}
...
In \Fig{fig:LHA_hadron_pp}, we show LHA distributions for the quark-enriched and gluon-enriched samples, using the default values $p_T^{\rm min} = 200~\GeV$ and $R = 0.4$.  For the quark-enriched $Z$ plus jets sample, the generators yield similar distributions, as expected since the different generators in the $e^+e^-$ study were in broad agreement on quark radiation patterns.  For the gluon-enriched dijet sample, the difference between generators is quite pronounced, yielding disagreements that are even larger than in the $e^+e^-$ study.  This difference is apparent also in the $\text{d} \Delta / \text{d} \lambda$ distribution, where \textsc{Pythia} predicts substantially larger separation power than \textsc{Herwig}, again in agreement with the $e^+e^-$ study.  Already from these raw distributions, we see that LHC jet shape measurements would help constrain power shower uncertainties, especially for gluon-enriched jets.

We next turn to the impact of jet grooming.  Often jet grooming is described as a strategy to mitigate jet contamination from pileup, underlying event, and initial state radiation.  Even at the level of final state radiation, though, grooming modifies the observed jet radiation patterns in ways that are interesting from the quark/gluon discrimination perspective.  This is shown for the LHA after mMDT in \Fig{fig:LHA_hadron_pp_mmdt}.   Because grooming removes soft peripheral radiation, it pushes the LHA to smaller values than in the ungroomed case.  If the parton showers differed primarily in their treatment of wide-angle soft radiation, then one would expect grooming to bring the distributions into closer agreement.  Instead, the differences between generators persists even after grooming, suggesting that the parton showers differ already in their treatment of collinear radiation, despite using the same underlying collinear splitting kernels.
...
The results of sweeping $p^\text{min}_T$ and $R$ are shown in \Fig{fig:pp_sweep}.  Increasing $p^\text{min}_T$ leads to a degradation of separation power, as minimum from the loss in quark/gluon-enrichment seen in \Fig{fig:parton_level_qg_composition}, though there are also relative differences between the parton showers that cannot be ascribed to sample composition alone.  Perhaps more interesting is the change in discrimination power with the jet radius $R$.  In \textsc{Pythia}, the discrimination is flat or even rising with increasing jet radius, whereas in \textsc{Herwig}, the discrimination power degrades noticeably at larger $R$.  This is true even after jet grooming, again pointing to differences in collinear physics and not just soft physics.  We conclude that varying $p_T^{\rm min}$ and $R$ provides important information that is not captured by looking at a single kinematic regime.
