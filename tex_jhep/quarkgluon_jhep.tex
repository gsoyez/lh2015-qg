...
\item \textsc{Vincia: alt $Q_E$}.  By default, \textsc{Vincia} uses a
  ...
  evolution variable for gluon emissions, which reproduces the logarithmic structure of the
  $q\bar{q}\to qg\bar{q}$ antenna function to second order in $\alpha_s$~\cite{Hartgring:2013jma}. This variation
...
to our knowledge, are not  used in any parton shower tuning strategy.
The relevant measurements include ... 
We strongly encourage the implementation of these
measurements in \textsc{Rivet}, which would enable them to be included
in the next round of parton-shower tunes and on the
