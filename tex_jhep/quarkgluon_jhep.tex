...
\ee
We mark this benchmark value on the subsequent plots for reference.\footnote{In large $N_c$ QCD where $C_A/C_F \to 2$, $\Delta \to \ln 3 - 1 \simeq 0.0986$, so quark/gluon separation is expected to be harder as $N_c \to \infty$}  \jdt{We don't yet, but we probably should.}  
...
\jdt{Andrew actually gave me NGL information, so we should include that.}
...
The IRC safe angularities are additive observables, meaning that one can decompose them into separate contributions from perturbative and nonperturbative modes:
\be
...
The shape function $F$ has to be extracted from data, but we can use a simple parametrization that accounts for some aspects of its known behavior \cite{}:
\be
F(\epsilon) = \frac{\pi \epsilon}{2 \epsilon_0^2} \exp\left[-\frac{\pi}{4} \frac{\epsilon^2}{\epsilon_0^2} \right].
\ee
This function goes linearly to zero at $\epsilon \to 0$, falls exponentially as $\epsilon \to \infty$, is normalized by $\int \df \epsilon \,  F(\epsilon) = 1$, and has an expectation value $\vev{\epsilon} = \int \df \epsilon \, \epsilon \, F(\epsilon) = \epsilon_0$.  The parameter $\epsilon_0$ can be interpreted as the average shift of the perturbative distribution from nonperturbative effects.\footnote{In \Ref{}, an alternative form of $F(\epsilon) = (4\epsilon/\epsilon_0^2) e^{-2 \epsilon/\epsilon_0}$ was proposed which has the same rough features.  We find that this alternative form has a large high-side tail and gives somewhat unrealistic angularity distributions for gluons.}
...
\ee
By tying $\Omega_0$ and $\Xi_0$ together, this has the effect of reducing the phase space for nonperturbative emissions from gluons, which is particularly important for $\beta < 1$.  Ideally, one would like a more rigorous justification for the assumptions in \Eq{eq:Xiscaling}.
...
Finally, our \textsc{Ariadne} baseline is based on a beta release of version 5.  In \Fig{fig:settings_variation_ariadne}, we consider the following \textsc{Ariadne} variation:
\begin{itemize}
